          ...
          <div className="overflow-auto" style={{ maxHeight: 320 }}>
            {favorites.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-center p-4">
                <div className="text-pablo-muted text-[12px] font-mono mb-2">
                  No assets in your watchlist
                </div>
                <div className="text-pablo-muted/60 text-[11px] font-mono">
              ...
              <table className="w-full text-[12px] font-mono">
                <thead className="text-pablo-muted uppercase tracking-wider sticky top-0 bg-pablo-panel z-10 text-[10px]">
                  <tr className="border-b border-pablo-border">
                    ...
                    <th className="px-3 py-2 text-right hidden sm:table-cell">MCap</th>
                    <th className="px-3 py-2 text-right w-16 hidden md:table-cell">7D</th>
                  </tr>
                </thead>
                <tbody>
                  {favorites.map(asset => {
                      ...
                      <tr key={asset.id} className="border-b border-pablo-border/20 hover:bg-pablo-green/5 transition-colors group">
                        <td className="px-1 py-2">
                          <button
                            ...
                            className="text-pablo-gold text-sm"
                          >
                            {'\u2605'}
                          </button>
                        </td>
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-2">
                            {asset.image && <img src={asset.image} alt="" className="w-5 h-5 rounded-full" loading="lazy" />}
                            <span className="text-pablo-light font-medium">{asset.symbol}</span>
                            <span className="text-pablo-muted text-[11px] hidden sm:inline">{asset.name}</span>
                          </div>
                        </td>
                        <td className="px-3 py-2 text-right text-pablo-light tabular-nums text-[12px]">{formatPrice(asset.price)}</td>
                        <td className={`px-3 py-2 text-right tabular-nums font-medium text-[12px] ${isPositive ? 'text-pablo-green' : 'text-red-400'}`}>
                          {isPositive ? '+' : ''}{asset.changePercent24h.toFixed(2)}%
                        </td>
                        <td className="px-3 py-2 text-right text-pablo-text tabular-nums text-[11px] hidden sm:table-cell">
          ...
          {global && (
            <div className="px-3 pb-3 space-y-2">
              ...
              <div className="flex justify-between items-center py-1">
                <span className="text-pablo-muted text-[10px] uppercase tracking-wider">MCap</span>
                <div className="flex items-center gap-2">
                  <span className="text-pablo-light text-[12px] font-mono font-medium">{formatNum(global.totalMarketCap)}</span>
                  <span className={`text-[10px] font-mono ${global.marketCapChange24h >= 0 ? 'text-pablo-green' : 'text-red-400'}`}>
                    {global.marketCapChange24h >= 0 ? '+' : ''}{global.marketCapChange24h.toFixed(2)}%
                  </span>
                </div>
              </div>
              <div className="flex justify-between items-center py-1">
                <span className="text-pablo-muted text-[10px] uppercase tracking-wider">24H Vol</span>
                <span className="text-pablo-light text-[12px] font-mono font-medium">{formatNum(global.totalVolume)}</span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="overflow-auto" style={{ maxHeight: 280 }}>
            {newsLoading && news.length === 0 ? (
              <div className="flex items-center justify-center h-20">
                <span className="text-pablo-green text-[11px] font-mono animate-pulse">Scanning...</span>
              </div>
            ) : (
                  ...
                  <div key={col} className="divide-y divide-pablo-border/20">
                    {news.slice(col * 5, col * 5 + 5).map(item => (
                      <a
                        ...
                        className="block px-3.5 py-2.5 hover:bg-pablo-green/5 transition-colors group"
                      >
                        <h3 className="text-[12px] text-pablo-light leading-relaxed group-hover:text-pablo-green transition-colors line-clamp-2 font-medium">
                          {item.title}
                        </h3>
                        <div className="flex items-center gap-1.5 mt-1.5">
                          <span className="text-[10px] text-pablo-green uppercase tracking-wider font-semibold">{item.source}</span>
...
function StatCell({ label, value, change, bar, barColor, tag, tagColor }: {
  ...
  return (
    <div className="stat-cell py-3">
      ...
      <div className="stat-value text-lg font-bold" style={tagColor ? { color: tagColor } : {}}>{value}</div>
      {change !== undefined && (
        <div className={`stat-change text-[11px] ${change >= 0 ? 'text-pablo-green' : 'text-red-400'}`}>
          {change >= 0 ? '+' : ''}{change.toFixed(2)}%
        </div>
      )}
      {tag && <div className="text-[10px] uppercase tracking-wider mt-1 font-semibold" style={{ color: tagColor }}>{tag}</div>}
      {bar !== undefined && (
        <div className="w-full max-w-[60px] bg-pablo-border rounded-full h-1.5 mt-2">
          <div className="h-1.5 rounded-full transition-all duration-500" style={{ width: `${bar}%`, backgroundColor: barColor }} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="py-0.5">
      <div className="flex justify-between items-center mb-1">
        <span className="text-pablo-muted text-[10px] uppercase tracking-wider">{label}</span>
        <span className="text-pablo-light text-[11px] font-mono font-medium">{value.toFixed(1)}%</span>
      </div>
